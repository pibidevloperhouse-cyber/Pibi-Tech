"use client";

import { TrendingUp, Target, Zap } from "lucide-react";

const PriceSenseRevenueImpact = () => {
  const outcomes = [
    {
      value: "15-25%",
      title: "Higher Win Rates",
      description: "Identify price bands that maximize deal success.",
      icon: Target,
    },
    {
      value: "8-18%",
      title: "Margin Improvement",
      description:
        "Reduce unnecessary discounting without hurting conversions.",
      icon: TrendingUp,
    },
    {
      value: "2-3x",
      title: "Faster Pricing Decisions",
      description: "Instant recommendations replace manual analysis.",
      icon: Zap,
    },
  ];

  return (
    <section className="section-padding bg-white">
      <div className="container-max">
        <div className="max-w-3xl mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            <span className="bg-clip-text text-transparent bg-brand-gradient">
              Measurable Revenue Outcomes
            </span>
          </h2>

          <p className="text-slate-700 text-xl leading-relaxed">
            Organizations using PriceSense see tangible improvements in pricing
            performance.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {outcomes.map((item, index) => {
            const Icon = item.icon;

            return (
              <div
                key={index}
                className="bg-white border border-slate-200 rounded-2xl p-8 hover:shadow-xl transition-all duration-300 text-center"
              >
                <div className="mx-auto w-14 h-14 flex items-center justify-center rounded-xl bg-[#248BB3]/10 mb-6">
                  <Icon className="w-7 h-7 text-[#248BB3]" />
                </div>

                <div className="mb-3">
                  <span className="text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-brand-gradient">
                    {item.value}
                  </span>
                </div>

                <h3 className="text-xl font-bold text-slate-900 mb-3">
                  {item.title}
                </h3>

                <p className="text-slate-600 text-md leading-relaxed">
                  {item.description}
                </p>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
};

export default PriceSenseRevenueImpact;
